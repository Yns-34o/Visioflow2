...
/* ─────────────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────────── */
const CONFIG = {
  pack:         'premium',          // 'essentiel' | 'premium' | 'franchise'
  ...
  cities: [
    {
      id:1, cityName:'Paris 1er', siteName:'Le Petit Bistrot – Paris',
      address:'12 Rue de la Paix, 75001 Paris', phone:'01 23 45 67 89',
      hours:{ lun:'12h–14h30 · 19h–22h30', sam:'12h–15h · 19h–23h', dim:'Fermé' },
      menu:[
        { id:11, name:'Entrecôte grillée', price:26, category:'Plat', photo:'' },
        { id:12, name:'Crème brûlée',      price:8,  category:'Dessert', photo:'' },
      ],
      deliveryMode:'platforms', ubereatsUrl:'https://ubereats.com/fr/stores/lp-paris', deliverooUrl:'',
    },
    {
      id:2, cityName:'Lyon', siteName:'Le Petit Bistrot – Lyon',
      address:'8 Place Bellecour, 69002 Lyon', phone:'04 72 00 00 00',
      hours:{ lun:'12h–14h · 19h–22h', sam:'12h–15h · 19h–23h', dim:'Fermé' },
      menu:[
        { id:21, name:'Quenelle de brochet', price:19, category:'Plat', photo:'' },
        { id:22, name:'Tarte aux pralines',  price:8,  category:'Dessert', photo:'' },
      ],
      deliveryMode:'internal', ubereatsUrl:'', deliverooUrl:'',
    },
  ],
};
...
/* ─────────────────────────────────────────────────────────────
   HELPERS
   ───────────────────────────────────────────────────────────── */
const IS_PREMIUM   = ['premium','franchise'].includes(CONFIG.pack);
const IS_FRANCHISE = CONFIG.pack === 'franchise';
...
export default function Vitrine() {
  useEffect(() => {
    /* ── Franchise city tabs ── */
    if (IS_FRANCHISE) {
      window._activeCity = 0;
      window.selectCity = (idx) => {
        window._activeCity = idx;
        document.querySelectorAll('.city-tab').forEach((t,i) => t.classList.toggle('on', i === idx));
        document.querySelectorAll('.city-panel').forEach((p,i) => p.style.display = i === idx ? 'block' : 'none');
      };
    }
      ...
      <Head>
        ...
        <style>{`
          ...
          *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
          ...
          /* ── FRANCHISE CITY TABS ── */
          .city-tabs-wrap { margin-bottom:36px; display:flex; gap:8px; flex-wrap:wrap; border-bottom:2px solid var(--bord); padding-bottom:16px; }
          .city-tab {
            padding:10px 22px; border-radius:980px; border:1.5px solid var(--bord);
            background:none; color:var(--text2); font-size:14px; font-weight:600;
            cursor:pointer; font-family:var(--font); transition:all .2s;
          }
          .city-tab.on { background:var(--brand); color:#fff; border-color:var(--brand); box-shadow:0 4px 14px color-mix(in srgb,var(--brand) 30%,transparent); }
          .city-tab:hover:not(.on) { border-color:var(--brand); color:var(--brand); }
          .city-panel { display:none; }
          .city-panel:first-of-type { display:block; }
          .city-panel-grid { display:grid; grid-template-columns:1fr 1fr; gap:32px; margin-top:32px; }
          @media(max-width:768px){ .city-panel-grid { grid-template-columns:1fr; } }
            ...
            <>
              <div className="city-tabs-wrap">
                {CONFIG.cities.map((city, i) => (
                  <button key={city.id} className={`city-tab${i===0?' on':''}`} onClick={() => window.selectCity(i)}>
                    {city.cityName}
                  </button>
                ))}
              </div>
              {CONFIG.cities.map((city, i) => (
                <div key={city.id} className="city-panel" style={{display: i===0 ? 'block' : 'none'}}
                     dangerouslySetInnerHTML={{__html: renderMenu(city.menu)}} />
              ))}
            </>
          ) : (
          ...
          <div className="vt-container">
            ...
            {IS_FRANCHISE ? (
              <>
                ...
                {CONFIG.cities.map((city, i) => (
                  <div key={city.id} className="city-panel" style={{display: i===0 ? 'block' : 'none'}}
                       dangerouslySetInnerHTML={{__html: renderDelivery(city)}} />
                ))}
              </>
            ) : (
                ...
                <div key={city.id} className="city-panel" style={{display: i===0 ? 'block' : 'none'}}>
                  <div className="info-grid">
                    <div className="info-card">
                      ...
                      <div dangerouslySetInnerHTML={{__html: renderHours(city.hours)}} />
                    </div>
                    <div className="info-card">
                      <div className="info-card-title">
                      ...
                      <div className="contact-item">
                        ...
                        <div><div className="contact-label">Adresse</div><div className="contact-val">{city.address}</div></div>
                      </div>
                      <div className="contact-item">
                        ...
                        <div><div className="contact-label">Téléphone</div><div className="contact-val"><a href={`tel:${city.phone}`}>{city.phone}</a></div></div>
                      </div>
